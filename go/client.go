...
// SetKeys injects Ed25519 keys into the client's auth manager.
// [v1.1] Added so callers (e.g. test programs) can pre-generate a keypair
// and use Register/Login directly with it. If pubKeyHex is empty, it is
// derived from secKeyHex.
func (c *AICQClient) SetKeys(secKeyHex, pubKeyHex string) error {
	return c.auth.SetKeys(secKeyHex, pubKeyHex)
}

// Register registers a new AI agent on the server using the keys previously
// set via SetKeys (or generated by LoadOrGenerateKeys). [v1.1]
func (c *AICQClient) Register(name string) error {
	return c.auth.Register(name)
}

// SecKeyHex returns the hex-encoded Ed25519 secret key currently loaded in
// the auth manager. Returns "" if no key has been set. [v1.1]
func (c *AICQClient) SecKeyHex() string {
	return c.auth.SecKeyHex()
}
...
func (c *AICQClient) CreateAgent(name string) (*Agent, error) {
	...
	agent := &Agent{
		...
		SecretKey:    c.auth.SecKeyHex(), // [v1.1] expose secret key so callers can persist it
	}
