...
// SecKeyHex returns the hex-encoded secret key.
// [v1.1] Added so callers (e.g. test programs, persistent agents) can
// retrieve the secret key after CreateAgent generates it.
func (a *AuthManager) SecKeyHex() string {
	a.mu.RLock()
	defer a.mu.RUnlock()
	if a.secKey == nil {
		return ""
	}
	return hex.EncodeToString(a.secKey)
}
